...
| Type | Status | Notes |
|---|---|---|
| `mssql` | ✅ Stable | SQL Server 2000+, Turkish_CI_AS, NOLOCK option |
| `rest` | ✅ Stable | OpenAPI/Swagger spec → typed MCP tools |
| `graphql` | ✅ Stable | Schema introspection → parameterized tools |
| `firebird` | 🚧 Placeholder | Adapter wired, implementation in progress |
| `postgres` / `mysql` | 🗺 Roadmap | Track [coremcp issues](https://github.com/corebasehq/coremcp/issues) |

Need a driver that isn't listed? CoreMCP exposes an [adapter interface](https://github.com/corebasehq/coremcp/tree/main/pkg/adapter) — community PRs welcome.
...
For parameterized tools with typed inputs, see [Configuration reference → custom_tools](/coremcp/configuration#custom-tools).
